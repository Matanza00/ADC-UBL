...
export default function ReportsTab({ entries, dailyRows, currentSite }) {
  ...
  const reportEntries = useMemo(() => entries.filter(e => e.invType === reportInvType), [entries, reportInvType]);
  ...
  /* ── Range ── */
  // NEW — two-stage: fine-grained ledger line first (correct opening/closing per product),
// then roll up into category+cardType totals by summing
const rangeData = useMemo(() => {
  if (!rangeFrom && !rangeTo) return [];

  const filtered = reportEntries.filter(e =>
    (!rangeFrom || e.date >= rangeFrom) &&
    (!rangeTo || e.date <= rangeTo) &&
    (!rangeCat || e.plasticCategory === rangeCat)
  );

  // Stage 1: group by actual ledger line
  const fineMap = {};
  ...
  filtered.forEach(e => {
    ...
    if (!r._firstDate || e.date < r._firstDate) {
      r._firstDate = e.date;
      r.opening = Number(e.openingBalance) || 0;
    }

    r.received += Number(e.receivedFromVendor) || 0;
    r.consumed += Number(e.totalConsumption) || 0;
    r.damaged += Number(e.damaged) || 0;
  });

  // Stage 2: Roll up to Plastic Category + Card Type
  const catMap = {};

  Object.values(fineMap).forEach(line => {
    const ck = `${line.plasticCategory}|||${line.cardType}`;
    ...
    r.consumed += line.consumed;
    r.damaged += line.damaged;
  });

  // Calculate closing instead of summing stored closing balances
...
const issuanceGrouped = useMemo(() => {
  // Stage 1: group by the true ledger line (matches ckCat/ckp identity)
  const fineMap = {};
  issuanceEntries.forEach(e => {
    const fk = `${e.invType || "PLASTIC"}|||${e.cardType}|||${e.scheme}|||${e.plasticCategory}|||${e.subProduct}|||${e.segment}`;
    const r = fineMap[fk] = fineMap[fk] || {
      invType: e.invType || "PLASTIC", plasticCategory: e.plasticCategory,
      opening: 0, received: 0, batchCount: 0, damaged: 0, extraCount: 0, closing: 0,
  ...
  // Stage 2: roll fine-grained lines up into category totals by summing correctly-computed values
  const catMap = {};
  Object.values(fineMap).forEach(line => {
    const ck = `${line.invType}|||${line.plasticCategory}`;
    const r = catMap[ck] = catMap[ck] || {
      invType: line.invType, plasticCategory: line.plasticCategory,
      opening: 0, received: 0, batchCount: 0, damaged: 0, extraCount: 0, total: 0, closing: 0,
    };
    r.opening += line.opening;
    r.received += line.received;
  ...
  Object.values(catMap).forEach(r => r.total = r.batchCount + r.extraCount);
  return catMap;
}, [issuanceEntries]);
  const issuanceTotals = useMemo(() => {
    const t = Object.values(issuanceGrouped).reduce((s, r) => ({
      opening: s.opening + r.opening, received: s.received + r.received, batchCount: s.batchCount + r.batchCount,
      damaged: s.damaged + r.damaged, extraCount: s.extraCount + r.extraCount, closing: s.closing + r.closing,
    }), { opening: 0, received: 0, batchCount: 0, damaged: 0, extraCount: 0, closing: 0 });
  ...
  const downloadIssuanceCSV = useCallback(() => {
    ...
    downloadStyledExcel({
      ...
      headers: ["S.No", "Plastic Type", "Opening", "Stock Received", "Batch Count", "Damage", "Extra Count", "Total", "Closing"],
      rows: dataRows.map((r, i) => [i + 1, r.plasticCategory, r.opening, r.received, r.batchCount, r.damaged, r.extraCount, r.total, r.closing]),
      footerRow: ["", "GRAND TOTAL", issuanceTotals.opening, issuanceTotals.received, issuanceTotals.batchCount, issuanceTotals.damaged, issuanceTotals.extraCount, issuanceTotals.total, issuanceTotals.closing],
  ...
  const downloadIssuancePDF = useCallback(() => {
    ...
    downloadStyledPDF({
      ...
      headers: ["S.No", "Plastic Type", "Opening", "Stock Received", "Batch Count", "Damaged", "Extra Count", "Total", "Closing"],
      rows: dataRows.map((r, i) => [i + 1, r.plasticCategory, fmt(r.opening), fmt(r.received), fmt(r.batchCount), fmt(r.damaged), fmt(r.extraCount), fmt(r.total), fmt(r.closing)]),
      footerRow: ["", "GRAND TOTAL", fmt(issuanceTotals.opening), fmt(issuanceTotals.received), fmt(issuanceTotals.batchCount), fmt(issuanceTotals.damaged), fmt(issuanceTotals.extraCount), fmt(issuanceTotals.total), fmt(issuanceTotals.closing)],
      filename: `Issuance_${issuanceType}_${pLabel || "All"}.pdf`,
      numericFromIndex: 2,
    });
      ...
      {activeReport === "issuance" && (
        <Card style={{ marginBottom: 24 }}>
          ...
          {!Object.keys(issuanceGrouped).length ? (
            ...
            <BaseTable headers={["Inv Type", "Plastic Category", "Opening", "Stock Received", "Batch Count", "Damaged", "Extra Count", "Total", "Closing"]} rightAlignIndex={1}>
